...
@interface GCViewHelper (Internal)
-(UIViewController*) getRootViewController;
@end

@implementation GCViewHelper

static GCViewHelper *instanceOfGCViewHelper;
...
+(id) alloc
{
	@synchronized(self)	{
		...
		NSAssert(instanceOfGCViewHelper == nil, @"Attempted to allocate a second instance of the singleton: GCViewHelper");
		instanceOfGCViewHelper = [[super alloc] retain];
		return instanceOfGCViewHelper;
	}
	return nil;
}
...
+(GCViewHelper*) sharedGCViewHelper
{
	@synchronized(self) {
		...
		if (instanceOfGCViewHelper == nil) {
		
			[[GCViewHelper alloc] init];
		}
...
-(void) dealloc
{
	[instanceOfGCViewHelper release];
	instanceOfGCViewHelper = nil;
...
+ (void)authenticateLocalPlayerWithCompletionHandler:(void(^)(NSError *error))completionHandler
{
    ...
    [localPlayer authenticateWithCompletionHandler:^(NSError *e) {
        if (localPlayer.isAuthenticated)
        {
			...
			GCLOG(@"Player authenticated (%@).", [localPlayer playerID]);
...
-(void) leaderboardViewControllerDidFinish:(GKLeaderboardViewController*)viewController
{
	//[GCCache dismissModalViewController];
